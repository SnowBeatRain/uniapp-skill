...
- **鸿蒙开发专题**：HarmonyOS App / 元服务开发全流程 → `references/harmony-development.md`
...
| `references/harmony-development.md` | 鸿蒙开发专题（17 章）：环境搭建、项目配置、签名权限、UTS 插件（arkts）、原生组件嵌入、华为登录、URL Scheme/App Linking、元服务开发、调试、发布、API 兼容速查、条件编译模板、WebView/导航栏/页面通信适配、老项目适配实战、**架构师深度笔记（JSVM 架构解析、第三方库兼容矩阵、CSS 渲染差异、键盘/暗黑/推送/生物认证/隐私合规/企业分发/生产调试/代码审查清单/已修复 Bug 史/30+ FAQ）** |
